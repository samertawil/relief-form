...
class MainController extends Controller
{
    public function main()
    {
        ...
        $address = address::select('id', 'address_type', 'active', 'home_type')->where('user_id', Auth::id())->where('address_type', 7)->first();
    ...
    public function getHelpstore(GetHelpRequest $request)
    {

        TechnicalSupport::create($request->all());
        return redirect()->route('login')->with('message', 'لقد تم استقبال طلبك بنجاح وسيتم فحص الطلب والتواصل معكم')->with('type', 'success');
    }
}
